...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt) and insect.istrue:
                return
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if self.place.ant is None or self.place.ant.blocks_path == False: # No ant in bee's place OR an is in bee's place, but blocks_path is False
            return False
        return True
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        return self.container and not self.ant and not other.container # Checks if ant is a container, ant does not contain another ant, and other ant is not a container (in respective order)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        distance = 0
        while (bee == None or distance < self.min_range) and (place is not hive):
            place = place.entrance
            bee = random_or_none(place.bees)
            distance = distance + 1

        if place is hive or distance > self.max_range: # Returns None if there is no bee whose place attribute is outside the Hive, or if distance is greater than max range
            return None
        else:
            return bee # Return nearest Bee, which could be the bee in the initialized location, or at a closer location, as generated in the for-loop
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        tempList = list(self.place.bees) # Create a new list to ensure that the for loop iterates through all the bees
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
            bee.reduce_armor(bee.armor) # Reduce the armor of the bee by bee.armor in order to remove bee from the list, since it has been consumed by the Ant

    def action(self, colony):
        if self.digesting:
            self.digesting -= 1
        else:
            newBee = random_or_none(self.place.bees) # Generate the next random Bee in its place
            if newBee:
                self.eat_bee(newBee) # HungryAnt eats the generated random Bee
                self.digesting = self.time_to_digest # Restart the digesting timer
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        # BEGIN Problem 8
        BodyguardAnt.action(self, colony) # Calls the action method from the base class BodyguardAnt, thus implementing the protection functionality of the TankAnt
        
        tempList = list(self.place.bees)
        for b in tempList:
            b.reduce_armor(self.damage)
...
class QueenAnt(ScubaThrower): 
    ...
    def __init__(self):
        ...
        if QueenAnt.real_queen:
            QueenAnt.real_queen = False # Account for impostors
            self.ants = [] # Keep track of all ants whose damage has been doubled
            self.istrue = True
        else:
            self.istrue = False
    ...
    def strengthen(self, ant):
        ...
        if ant not in self.ants:
            ant.damage *= 2
            self.ants.append(ant) # Append to self.ants to ensure that same ant isn't damaged twice

        if ant.container: # Same process if container ant
            contain_ant = ant.ant
            if contain_ant:
                if contain_ant not in self.ants:
                    contain_ant.damage *= 2
                    self.ants.append(contain_ant) # Append to self.ants to ensure that same ant isn't damaged twice
    ...
    def action(self, colony):
        ...
        while place.exit:
            place = place.exit
            self.strengthen(place.ant)

        if len(place.bees) > 0:
            bees_win()

        ScubaThrower.action(self, colony) # Call base class action method, since QueenAnt Class inherits from ScubaThrower Ant Class
