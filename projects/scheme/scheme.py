...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        proc = scheme_eval(first, env) # Gets the type of procedure based on the operator (+: addition, -:substraction, /:division, etc.)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(vals) > len(formals):
            raise SchemeError("Too many vals given")
        elif len(formals) > len(vals):
            raise SchemeError("Too few vals given")
        else:
            tempFormals = formals
            tempVals = vals

            for i in range(len(tempFormals)):
                child.define(tempFormals.first, tempVals.first)
                tempFormals = tempFormals.second
                tempVals = tempVals.second
        return child
...
class LambdaProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        ...
        tempFormals = self.formals
        tempArgs = args

        for i in range(len(tempFormals)):
            frame.define(tempFormals.first, tempArgs.first)
            tempFormals = tempFormals.second
            tempArgs = tempArgs.second
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(target.first, make_lambda_function)
        return target.first
    else:
