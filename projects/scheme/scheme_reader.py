...
class Pair:
    ...
    def map(self, fn):
        ...
        mapped = fn(self.first) # mapped = fn(rest.first)
        if self.second is nil or isinstance(self.second, Pair): # if rest.second is nil
...
def read_tail(src):
    ...
    try:
        ...
        elif src.current() == ')': # In this case, the original src sequence would have contained '()'
            src.remove_front()
            return nil
        elif src.current() == '.': # 2 . 3)
            src.remove_front()  # 3)
            checkFirst = scheme_read(src) # val = 3; src = )
            checkRest = read_tail(src) # )
            if checkRest == nil:
                return checkFirst
            else:
                raise SyntaxError('Only one item can appear after the dot')
            # END PROBLEM 2
        else:
            # BEGIN PROBLEM 1
            firstNext = scheme_read(src)
            restNext = read_tail(src)
            reprPair = Pair(firstNext, restNext)
            return reprPair
            # END PROBLEM 1
    except EOFError:
        raise SyntaxError('unexpected end of file')
