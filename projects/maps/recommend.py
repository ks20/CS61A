...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        # BEGIN Question 6
        iter_cluster = group_by_centroid(restaurants, old_centroids)
        centroids = [find_centroid(cluster) for cluster in iter_cluster]
        # END Question 6
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    temps_s_xx = sum([x - mean(xs) for x in xs])
    temps_s_yy = sum([y - mean(ys) for y in ys])
    zipxy = zip(xs, ys)
    s_xy = sum([(x - mean(xs)) * (y - mean(ys)) for x, y in zipxy])
    #s_xy = temps_s_xx * temps_s_yy
...
def best_predictor(user, restaurants, feature_fns):
    ...
    '''maximum = distance(location, feature_fns[0])
    maximumFeature = feature_fns[0]
    for feature in feature_fns:
        y = find_predictor(user, reviewed, feature)
        if y > maximum:
            maximumFeature = y
    return maximumFeature'''

    return max([find_predictor(user, reviewed, feature) for feature in feature_fns], key = lambda num: num[1])[0]
    # END Question 8
